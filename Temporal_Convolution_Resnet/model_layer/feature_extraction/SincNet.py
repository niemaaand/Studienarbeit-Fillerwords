# https://github.com/mravanelli/SincNet/blob/d74164861d8bd8ab9cb02f102d677f5605edfbf4/dnn_models.py
...
class SincConv_fast(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size, sample_rate=16000, in_channels=1,
                 stride=1, padding=0, dilation=1, bias=False, groups=1, min_low_hz=20, min_band_hz=20):
        ...
        mel = np.linspace(self.to_mel(self.min_low_hz),
                          self.to_mel(high_hz),
                          self.out_channels + 1)
        hz = self.to_hz(mel)
...
class SincNet(nn.Module):
    def __init__(self, options):
        ...
        if self.cnn_use_laynorm_inp:
            self.ln0 = nn.LayerNorm(self.input_dim) #LayerNorm(self.input_dim)
        ...
        for i in range(self.N_cnn_lay):
            ...
            # layer norm initialization
            self.ln.append(
                nn.LayerNorm([int(self.cnn_N_filt[i]), int((current_input - self.cnn_len_filt[i] + 1) / self.cnn_max_pool_len[i])]))
                #LayerNorm([N_filt, int((current_input - self.cnn_len_filt[i] + 1) / self.cnn_max_pool_len[i])]))

            self.bn.append(
                nn.BatchNorm1d(int(self.cnn_N_filt[i]), int((current_input - self.cnn_len_filt[i] + 1) / self.cnn_max_pool_len[i]),
                               momentum=0.05))
        ...
        self.out_dim = current_input * int(self.cnn_N_filt[i])
